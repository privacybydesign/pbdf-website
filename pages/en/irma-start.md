...
header:
  ...
  title: Starting with the Yivi app
...
These page provides answers to the following questions.
 ...
 1. [How to get started with Yivi](#getstarted)
 2. [What does MyYivi provide?](#myirma)
 3. [What happens under the hood?](#hood)

Knowing about this last point is not essential for using Yivi; it only
provides additional background information for people who wish to
understand more about Yivi.

Deze pagina geeft antwoord op de volgende vragen.

### <a name="getstarted"></a>1. How to get started with Yivi

The next diagram summarizes the initial steps, when you wish to start
using Yivi.

<p align="center"><img src="../images/Registratie_Stappenplan_en.png" alt="Yivi issuer" style="width: 50%; height: 50%"/></p>
...
passport, Yivi is strictly personal: no-one else should be able to
(ab)use your Yivi identity.

#### 1.1. Installation

The Yivi app is available via the [Download](/download-en) page of the
foundation. You can also directly install the Yivi app on your phone
or tablet via the Android Playstore or the Apple App store.

#### 1.2. Registration

When you open the Yivi app for the first time, you have to set it up.
You are asked to provide:

 * a PIN code of 5 digits

Please choose your PIN code with care. You will regularly need it when
you use the Yivi app. This code guarantees that you are the only
person who is using your Yivi app. Avoid obvious codes like 00000. If
...
Every time you authenticate with Yivi, by revealing Yivi attributes,
...
registration at the foundation, on the [MyYivi](/myirma) webpage.
...
When you choose to associate an email address to your Yivi app, please
...
about usage of Yivi. The address is not shared with anyone else.


#### 1.3. Personalization with your email address

The email address that you possibly submit upon registration can now
be loaded into your Yivi app on your phone. After the previous
...
will be added as attribute in your Yivi app.
 ...
 * When you open the webpage in a browser on a PC or laptop and push
   the "Email Issue" button, a QR code appears. Now open the Yivi app
 ...
 * When you open the webpage in a browser on your phone itself and
   press the "Email Issue" button, the Yivi app on your phone opens
   automatically. You are also asked if you wish to accept your email
   address as attribute on your phone.

After registration you can try out the [YiviTube demo](/demo/irmaTube).
...
later. To do so, please visit the [Yivi issuance](/issuance) page.
...
as more parties join Yivi. At this stage the focus is on attributes
from Dutch sources. If you wish to join with other, possibly
international, attributes, please do [contact](/contact-en) the
foundation.


### <a name="myirma"></a>2. What does MyYivi provide?

The [MyYivi](/myirma) website of the foundation offers the following
possibilities.

* You can monitor the usage of your own Yivi app: the *log* files can
  ...
  brings us to the second possibility that the myYivi webpage offers.

* At the MyYivi webpage you can disable (block) further usage of your
  Yivi app. Of course, you can do this at any moment when you no
  longer wish to use Yivi. In addition, however, it is important to be
  able to pull this "emergence brake" when your phone has been stolen,
  so that no-one else can abuse your identity.

  When you decide to terminate Yivi on the MyYivi webpage, all your
  data will be deleted immediately, so that your Yivi app can no
  longer be used, even if it is sill installed on your phone. If, at
  some later stage, you choose to start using Yivi again, you have to
  re-register from the start.

* At the MyYivi webpage you can also manage which email address, if
  any, is associated with your Yivi app.

Logging into the MyYivi webpage can be done in two ways:

1. With Yivi itself, via your email attribute.
2. By typing in your email address; you then receive a fresh link by email,
   which gives you access to your MyYivi.

The latter, second option can only be used when you have (earlier)
associated an email address with your Yivi app. This is especially
...
login from there into MyYivi and disable your Yivi app.

Because of this second option it is wise to associate an email
address with your Yivi app.


### <a name="hood"></a>3. What happens under the hood?

The text below dives deeper into what precisely happens when you
register via the Yivi app. This background information is not
necessary for actual usage of Yivi; it is intended for people who are
...
Yivi ecosystem. Even more information is available on a [separate
page](/irma-explanation).

One essential point is that the Yivi app is strictly personal and
...
The Yivi implementation uses a *split key* trick, so that secret
cryptographic information is distributed between the app and the
MyYivi server of the foundation.  The app and the server have to
collaborate in a precisely defined manner in order to make Yivi
...
As a result, the MyYivi server alone can never pretend to be you: the
app on your phone is strictly required. What you can do on the server
side is described [above](#myirma). Logging into the MyYivi server can
be done via Yivi, but also via a link that is sent by email. If your
phone has been stolen, the thief probably also has access to your
email. Hence he/she can log into your MyYivi. But the only
thing that the thief can do there is disable your Yivi app. Hopefully,
by then, you have already done this yourself.

To summarize: the MyYivi server offers additional protection and
inspection, but cannot do anything on its own --- except disable.  The
Privacy by Design foundation operates the MyYivi server in order enable
the usage of Yivi. In principle, other parties can run such a server
too.

The MyYivi server also helps to protect your PIN code. In general, it
is unwise to store a PIN inside an app on a phone, because it could
possibly be extracted when your phone is hacked. Therefore, the Yivi
app does not store your PIN locally. Instead, it stores a large
arbitrary number, called a *nonce*. Also the MyYivi server does not
...
MyYivi server, and deletes your PIN from its memory. If the hash value
...
can possibly extract the nonce from your Yivi app, but that is not
...
for personalization of your Yivi app. On the other hand, it operates
the MyYivi server, for inspection of your own Yivi usage and for
possibly blocking your Yivi app.

<p align="center"><img src="../images/Rollen_Stichting_en.png" alt="Yivi
uitgever" style="width: 70%; height: 70%"/></p>

Initially, the foundation is the only issuer of Yivi
attributes. Hopefully, other parties will follow.

The foundation also verifies attributes, but only for itself, when
Yivi users log into MyYivi. Such verifications are not done for
